...
function TreeBranch({
  ...
  const [open, setOpen] = React.useState(onActivePath || descendantActive);

  // 展开全部：信号递增时强制展开（覆盖手动与 active 路径状态）
  const lastExpand = React.useRef(expandSignal);
  React.useEffect(() => {
  ...
  // 合并全部：信号递增时强制收起（手动打开的、以及 active 路径都一并合回）
  const lastCollapse = React.useRef(collapseSignal);
  React.useEffect(() => {
  ...
  // 选中节点时自动展开其祖先链，便于定位（仅重新选中时触发，不干扰合并全部）
  React.useEffect(() => {
    if (onActivePath || descendantActive) setOpen(true);
  }, [activeId]);
    ...
    return (
      <li>
        ...
        {open && (
          <ul>
            {node.children!.map((c) => (
              <TreeBranch key={c.id} node={c} depth={depth + 1} activeId={activeId} activePathIds={activePathIds} expandSignal={expandSignal} collapseSignal={collapseSignal} onSelect={onSelect} />
            ))}
          </ul>
        )}
      </li>
...
export function FieldTreeNav({
  ...
  const { t } = useI18n();
  // nextAction 表示「下一次点击按钮要执行的动作」，按钮文案随其变化
  const [nextAction, setNextAction] = React.useState<"expand" | "collapse">("expand");
  ...
  const onToggle = () => {
    if (nextAction === "expand") {
      ...
      setNextAction("collapse");
    } else {
      setCollapseSignal((s) => s + 1);
      setNextAction("expand");
    }
  };
  ...
  return (
    <nav className="overflow-hidden rounded-xl border border-border/60">
      <div className="flex items-center justify-between border-b border-border/40 bg-muted/30 px-3 py-2">
        ...
        >
          {nextAction === "expand" ? t("field_tree_expand") : t("field_tree_collapse")}
        </button>
      </div>
      <ul className="p-1.5">
        {fieldTree.map((n) => (
          <TreeBranch key={n.id} node={n} depth={0} activeId={activeId} activePathIds={activePathIds} expandSignal={expandSignal} collapseSignal={collapseSignal} onSelect={onSelect} />
        ))}
      </ul>
    </nav>
  );
